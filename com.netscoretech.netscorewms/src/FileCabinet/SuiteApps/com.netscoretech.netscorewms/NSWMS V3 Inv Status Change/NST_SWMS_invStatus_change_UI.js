  ...
  const onRequest = (scriptContext) => {
    try {
      ...
      urlObj.ajaxUrl = utility.getScriptUrl(
        "customscript_nst_wms_invstatus_change_be",
        "customdeploy_nst_wms_invstatus_change_be"
      );
      var Html,
        filesObj = utility.getFilesInFolder("NSWMS V3 Inv Status Change", true),
        globalFiles = utility.getFilesInFolder("NSWMS V3 Globals", true),
        images = utility.getFilesInFolder("NSWMS V3 Images", true);
      Html = https.get({
        url: filesObj["NST_SWMS_invStatus_change.html"],
      });
      ...
      HtmlDocument = HtmlDocument.replace(
        "customJs",
        filesObj["NST_SWMS_invStatus_change_Assets.js"]
      );
      ...
      log.error("Catch Block Message In nst_wms_BinCount_UI::", e);
    }
  };
