  ...
  const onRequest = (scriptContext) => {
    try {
      ...
      urlObj.ajaxUrl = utility.getScriptUrl(
        "customscript_nst_wms_cartputaway_be",
        "customdeploy_nst_wms_cartputaway_be"
      );
      var Html,
        filesObj = utility.getFilesInFolder("NSWMS V3 Cart PutAway", true),
        globalFiles = utility.getFilesInFolder("NSWMS V3 Globals", true),
        images = utility.getFilesInFolder("NSWMS V3 Images", true);
      Html = https.get({
        url: filesObj["NST_SWMS_CartPutAway.html"],
      });
      ...
      HtmlDocument = HtmlDocument.replace(
        "customJs",
        filesObj["NST_SWMS_CartPutAway_Assets.js"]
      );
      ...
      log.error("Catch Block Message In nst_wms_CartPutAway_UI::", e);
    }
  };
