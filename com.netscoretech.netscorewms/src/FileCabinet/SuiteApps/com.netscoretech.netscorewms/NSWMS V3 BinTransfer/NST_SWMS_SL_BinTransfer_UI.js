  ...
  const onRequest = (scriptContext) => {
    try {
      ...
      urlObj.ajaxUrl = utility.getScriptUrl(
        "customscript_nst_wms_bintransfer_be",
        "customdeploy_nst_wms_bintransfer_be"
      );
      var Html,
        filesObj = utility.getFilesInFolder("NSWMS V3 BinTransfer", true),
        globalFiles = utility.getFilesInFolder("NSWMS V3 Globals", true),
        images = utility.getFilesInFolder("NSWMS V3 Images", true);
      Html = https.get({
        url: filesObj["NST_SWMS_BinTransfer.html"],
      });
      ...
      HtmlDocument = HtmlDocument.replace(
        "customJs",
        filesObj["NST_SWMS_BinTransfer_Assets.js"]
      );
      ...
      log.error("Catch Block Message In nst_wms_SL_BinTransfer_UI::", e);
    }
  };
