  ...
  const onRequest = (scriptContext) => {
    try {
      ...
      urlObj.ajaxUrl = utility.getScriptUrl(
        "customscript_nst_wms_pack_shiporder_be",
        "customdeploy_nst_wms_pack_shiporder_be"
      );
      urlObj.packList = utility.getScriptUrl(
        "customscript_nst_wms_packlist_page",
        "customdeploy_nst_wms_packlist_page"
      );
      var Html,
        filesObj = utility.getFilesInFolder("NSWMS V3 Pack-Ship Order", true),
        globalFiles = utility.getFilesInFolder("NSWMS V3 Globals", true),
        images = utility.getFilesInFolder("NSWMS V3 Images", true);
      Html = https.get({
        url: filesObj["NST_SWMS_Pack_ShipOrder_Template.html"],
      });
      ...
      HtmlDocument = HtmlDocument.replace(
        "customJs",
        filesObj["NST_SWMS_Pack_ShipOrder_Assets.js"]
      );
