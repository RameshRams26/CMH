  ...
  const onRequest = (scriptContext) => {
    try {
      ...
      urlObj.ajaxUrl = utility.getScriptUrl(
        "customscript_nst_wms_invadjustment_be",
        "customdeploy_nst_wms_invadjustment_be"
      );
      urlObj.logIn = utility.getScriptUrl(
        "customscript_nst_wms_login_page",
        "customdeploy_nst_wms_login_page"
      );
      urlObj.adjUrl = utility.getScriptUrl(
        "customscript_nst_wms_crt_inv_adjust",
        "customdeploy_nst_wms_crt_inv_adjust"
      );
      var Html,
        filesObj = utility.getFilesInFolder("NSWMS V3 Inv Adjustment", true),
        globalFiles = utility.getFilesInFolder("NSWMS V3 Globals", true),
        images = utility.getFilesInFolder("NSWMS V3 Images", true);
      Html = https.get({
        url: filesObj["NST_SWMS_InvAdjustment_Template.html"],
      });
      var HtmlDocument = Html.body;
      HtmlDocument = HtmlDocument.replace(
        "customJs",
        filesObj["NST_SWMS_InvAdjustment_Assets.js"]
      );
      ...
      log.error("Catch Block Message In nst_wms_SL_InvAdjustment_UI::", e);
    }
  };
