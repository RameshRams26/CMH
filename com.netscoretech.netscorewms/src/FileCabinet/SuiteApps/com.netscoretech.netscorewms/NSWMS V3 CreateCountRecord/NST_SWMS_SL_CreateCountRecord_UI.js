  ...
  const onRequest = (scriptContext) => {
    try {
      ...
      urlObj.ajaxUrl = utility.getScriptUrl(
        "customscript_nst_wms_cntrecd_be",
        "customdeploy_nst_wms_cntrecd_be"
      );
      var Html,
        filesObj = utility.getFilesInFolder("NSWMS V3 CreateCountRecord", true),
        globalFiles = utility.getFilesInFolder("NSWMS V3 Globals", true),
        images = utility.getFilesInFolder("NSWMS V3 Images", true);
      Html = https.get({
        url: filesObj["NST_SWMS_CreateCountRecord.html"],
      });
      ...
      HtmlDocument = HtmlDocument.replace(
        "customJs",
        filesObj["NST_SWMS_CreateCountRecord_Assets.js"]
      );
      ...
      log.error("Catch Block Message In nst_wms_SL_CreateCountRecord_UI::", e);
    }
  };
