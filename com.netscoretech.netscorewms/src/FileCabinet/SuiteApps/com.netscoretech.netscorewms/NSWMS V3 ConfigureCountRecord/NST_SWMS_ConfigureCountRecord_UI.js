 ...
 */ (https, search, url, utility) => {
  ...
  const onRequest = (scriptContext) => {
    try {
      ...
      var tranId = scriptContext.request.parameters.tranId;
      var urlObj = {
        ajaxUrl: "",
        setUpId: setUpId,
        empId: empId,
      ...
      urlObj.ajaxUrl = utility.getScriptUrl(
        "customscript_nst_wms_confgrcntrecd_be",
        "customdeploy_nst_wms_confgrcntrecd_be"
      );
      urlObj.invcountlist = utility.getScriptUrl(
        "customscript_nst_wms_invcountlist_page",
        "customdeploy_nst_wms_invcountlist_page"
      );
      var Html,
        filesObj = utility.getFilesInFolder(
          "NSWMS V3 ConfigureCountRecord",
          true
        ),
        globalFiles = utility.getFilesInFolder("NSWMS V3 Globals", true),
        images = utility.getFilesInFolder("NSWMS V3 Images", true);
      Html = https.get({
        url: filesObj["NST_SWMS_ConfigureCountRecord.html"],
      });
      ...
      HtmlDocument = HtmlDocument.replace(
        "customJs",
        filesObj["NST_SWMS_ConfigureCountRecord_Assets.js"]
      );
      ...
      log.error("Catch Block Message In nst_wms_ConfigureCountRecord_UI::", e);
    }
  };
