  ...
  const onRequest = (scriptContext) => {
    try {
      ...
      urlObj.ajaxUrl = utility.getScriptUrl(
        "customscript_nst_wms_receiverma_be",
        "customdeploy_nst_wms_receiverma_be"
      );

      var Html,
        filesObj = utility.getFilesInFolder("NSWMS V3 ReceiveRMA", true),
        globalFiles = utility.getFilesInFolder("NSWMS V3 Globals", true),
        images = utility.getFilesInFolder("NSWMS V3 Images", true);
      Html = https.get({
        url: filesObj["NST_SWMS_ReceiveRMA_Template.html"],
      });
      ...
      HtmlDocument = HtmlDocument.replace(
        "customJs",
        filesObj["NST_SWMS_ReceiveRMA_Assets.js"]
      );
      ...
      log.error("Catch Block Message In nst_wms_SL_ReceiveRMA_UI::", e);
    }
  };
